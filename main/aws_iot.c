...
#define AWS_IOT_TASK_STACK_SIZE  10240
...
const char esp_pub_topic[] = "esp/pub";
const char esp_sub_topic[] = "esp/sub";
...
void aws_iot_task(void *param) {
   ...
   // const char *TOPIC = "test_topic/esp32";
    const int TOPIC_LEN = strlen(esp_sub_topic);

    ESP_LOGI(TAG, "Subscribing...");
    rc = aws_iot_mqtt_subscribe(&client, esp_sub_topic, TOPIC_LEN, QOS0, iot_subscribe_callback_handler, NULL);
    if(SUCCESS != rc) {
    ...
    while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)) {
        ...
        rc = aws_iot_mqtt_publish(&client, esp_pub_topic, strlen(esp_pub_topic), &paramsQOS0);

        sprintf(cPayload, "{\"%s\" : %.1f, \"%s\" : %.1f}", "temperature",(float)rand(), "humidity", (float)rand());
        paramsQOS1.payloadLen = strlen(cPayload);
        rc = aws_iot_mqtt_publish(&client, esp_pub_topic, strlen(esp_pub_topic), &paramsQOS1);
